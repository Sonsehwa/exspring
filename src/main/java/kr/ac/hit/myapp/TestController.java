...
//		@RequestMapping("/test/b.do")
		public String bbb(int x, @RequestParam("y") int b, Map m) {
			//전송되어 오는 파라미터 이름과 동일한 이름의 변수를 인자로 선언하여 파라미터 값을 받을 수 있다.
			//변수타입에 맞게 자동으로 형변환
			...
			//해당 Map, Model, ModelMap변수에 데이터를 저장하여 JSP에 전달 가능
		...
		//@ModelAttribute()를 생략시 타입이름(첫글자만 소문자로 변환)으로 모델에 자동 저장
		@RequestMapping("/test/b.do")
